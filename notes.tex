...
Given a context $\Gamma$ and a type $\sigma$ we define the set of terms $\Tm(\Gamma,\sigma)$ inductively by the following rules:
\begin{align*}
  & \infer{0 : \Tm(\Gamma\rhd\sigma,\sigma)}{}
  \qquad
  \infer{1+t : \Tm(\Gamma\rhd\sigma,\tau)}{t : \Tm(\Gamma,\tau)}\\\\
...
\end{align*}
We write the de Bruijn index $2$ as $1+1+0$. In our syntax we also have terms like $1+(0\,1)$ which should be equivalent to $1\,2$.
\begin{question}
What should $1 + \lambda\,(0\, 1)$ correspond to using proper indices?
\end{question}
We will address the question how to make this correspondence precise below.
...
where $t_i : \Tm(\Gamma,\tau_i)$. Given $t_n,t_{n-1},\dots t_0 : \Subst(\Gamma,\Delta)$ we write $\epsilon : \Subst(\Gamma,\bullet)$ for the empty substitution.

Given $\vec{t}$ as above, we can apply $1+$ to it componentwise:
\begin{align*}
  & 1 + \vec{t} : \Subst (\Gamma.\sigma,\Delta) \\
  & 1 + \vec{t} = 1+t_n , 1+t_{n-1} , \dots , 1+t_0
\end{align*}

Using $1+\vec{t}$ we can define lifting which will be useful when shifting a substitution under a $\lambda$:
\begin{align*}
& \vec{t} \rhd \sigma  : \Subst(\Gamma \rhd\sigma,\Delta\rhd\sigma)\\
& \vec{t} \rhd \sigma  = (1 + \vec{t},  0)
\end{align*}

We define parallel substitution for terms: given $t : \Tm(\Gamma,\sigma)$ and $\vec{u} : \Subst(\Delta,\Gamma)$ we define $t[\vec{u}] : \Tm(\Delta,\sigma)$ by the following equations:
  
  \begin{eqnarray*}
  0 [ \vec{u},u ] & = & u \\ 
  (1+t) [ \vec{u},u ]& = & t [\vec{u} ]\\
  (t\,v) [\vec{u}] & = & (t [\vec{u}]) \,(v[\vec{u}) \\
  (\lambda^\sigma t) [\vec{u}] & = & \lambda (t [\vec{u} \rhd \sigma])    
  \end{eqnarray*}

We now define identity and composition : Given $\Gamma = \sigma_n . \sigma_{n-1} \dots .\sigma_0$ we define
\begin{align*}
  & \id_\Gamma  :   \Subst(\Gamma,\Gamma) \\
  & \id_\Gamma = n , n-1 , \dots, 0 
\end{align*}

Now given $\vec{t} = t_n,t_{n-1},\dots, t_0 : \Subst(\Gamma,\Delta)$ and $\vec{u} : \Subst(\Theta,\Gamma)$ we define
\begin{align*}
  & \vec{t} \circ \vec{u} : \Subst(\Theta,\Delta) \\
  & \vec{t} \circ \vec{u}  = t_n[\vec{u}] , t_{n-1}[\vec{u}], \dots t_0[\vec{u}]
\end{align*}

 We can now add the equations which allow us to weird terms like $1+(0\,1)$ or $1 + \lambda\,(0 1)$ to one with proper numerical indices -- this is sometimes called \emph{$\alpha$-normalisation}.

  \begin{eqnarray*}
    1+(t\,u) & = & (1+t) \, (1+u) \\
    1+(\lambda^\sigma t) & = & \lambda (t [(1 + \id) \rhd \sigma]
  \end{eqnarray*}

In the equation for $\lambda$ we weaken under a binder, that is we use $(1 + \id) \rhd \sigma : \Subst(\Gamma\rhd\tau\rhd\sigma,\Gamma\rhd\sigma)$. 
