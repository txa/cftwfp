...
\[ \lambda  x . \lambda y  . \lambda z . x\,z\,(y\,z) : (\sigma \to \tau \to \rho) \to (\sigma \to \tau) \to \sigma \to \rho \]
is written as
...
We are going to use an intrinsic syntax, that is we define typed terms only. For this we need the notion of a context which for de Bruijn terms is just a sequence of types, e.g. $\Gamma = \sigma_n . \sigma_{n-1} \dots .\sigma_0$, is a typical context; we write $\bullet$ for the empty context. We extend contexts on the right, that is they are snoc-lists. 

Given a context $\Gamma$ and a type $\sigma$ we define the set of terms $\Tm(\Gamma,\sigma)$ inductively by the following rules:
\begin{align*}
  & \infer{0 : \Tm(\Gamma.\sigma,\sigma)}{}
  \qquad
  \infer{1+t : \Tm(\Gamma.\sigma,\tau)}{t : \Tm(\Gamma,\tau)}\\
  & \infer{t\,u : \Tm(\Gamma,\tau)}{t : \Tm(\Gamma,\sigma\to\tau) \quad u : \Tm(\Gamma,\sigma)}
  \qquad
  \infer{\lambda t : \Tm(\Gamma,\sigma\to\tau)}{t : \Tm(\Gamma.\sigma,\tau)} 
\end{align*}
We write the de Bruijn index $2$ as $1+1+0$. In our syntax we also have terms like $1+(0\,1)$ which can be reduced to $1\,2$.
\begin{question}
Can you reduce to $1 + \lambda(0 1)$ to a term ony using proper indices?
\end{question}
...
Given $\vec{t}$ as above, we write $1 + \vec{t} : \Subst (\Gamma.\sigma,\Delta)$ for $1+t\_n , 1+t_{n-1} , \dots , 1+t_0$.
  ...
  (\lambda t) [\vec{t}] & = & \lambda (t [1+\vec{t},0])    
  \end{eqnarray*}

  We can now define identity and substitutions: Given $\Gamma = \sigma_n . \sigma_{n-1} \dots .\sigma_0$ we define
  ...
  \begin{exercise}
    Show that this lambda terms and substitutions form a category $\cat{\Lambda}$ where the objects are contexts, given contexts $\Gamma,\Delta$ the homsets are substitutions $\Subst(\Gamma,\Delta)$ and identity and composition are defined as above. You need to check the laws.

      As lemmas it is useful to establish:
      \begin{eqnarray*}
        t [\id] & = & t \\
